...
//!
//! Fixtures are recorded with `cargo run -p spankd -- record-suite`, and thresholds are
//! chosen with `cargo run -p spankd -- sweep`. The suite is skipped rather than failed
...
fn detect(fixture: &Fixture, cfg: Config) -> Vec<spank_dsp::Detection> {
...
fn everyday_activity_is_silent() {
    let rows = score();
    if rows.is_empty() {
        eprintln!("no fixtures recorded — run `cargo run -p spankd -- record-suite`");
        return;
    }
