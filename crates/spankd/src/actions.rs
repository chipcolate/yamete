        ...
        let job = match &action.kind {
            ...
            ActionKind::Exec {
                ...
                Job::Exec {
                    ...
                    args: args
                        .iter()
                        .map(|a| spank_proto::config::render(a, &vars))
                        .collect(),
            ...
            ActionKind::Webhook {
                ...
                let vars = template_vars(slap);
                Job::Webhook {
                    url: spank_proto::config::render(url, &vars),
                    method: method.to_uppercase(),
                    headers: headers
                        .iter()
                        .map(|(k, v)| (k.clone(), spank_proto::config::render(v, &vars)))
                        .collect(),
                    body: match body {
                        Some(t) => spank_proto::config::render(t, &vars),
...
mod tests {
    ...
    fn delay_is_generic_across_every_action_kind() {
        ...
        for kind in [
            ...
            assert!(spank_proto::DaemonConfig {
                actions: vec![action],
                ..Default::default()
            }
            .validate()
    ...
    fn an_absurd_delay_is_rejected() {
        let config = spank_proto::DaemonConfig {
            actions: vec![Action {
                id: "slow".into(),
                delay_ms: 60_000,
                ..Default::default()
