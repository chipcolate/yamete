...
use spank_sensor::Error;

/// launchd job label, and the plist filename.
pub const LABEL: &str = "com.chipcolate.spank";
...
<dict>
	...
	<string>{log_dir}/spankd.log</string>
	<key>StandardErrorPath</key>
	<string>{log_dir}/spankd.err.log</string>
</dict>
</plist>
"#,
        label = LABEL,
...
pub fn install(program: Option<PathBuf>, copy: bool) -> Result<(), Error> {
    ...
    let program = if copy {
        let dest = default_install_dir().join("spankd");
    ...
    println!("  logs     {}/spankd.log", logs.display());
    println!("\nIt will start at login and restart if it crashes.");
    println!("Check it with `spankd status`, stop it with `spankd uninstall`.");
    println!("To update: rebuild, then `spankd install --copy` again.");
    Ok(())
}
...
pub fn restart() -> Result<(), Error> {
    if !is_loaded() {
        return Err(Error::Iokit(format!(
            "{LABEL} is not installed — run `spankd install` first"
        )));
    }
...
pub fn show() -> Result<(), Error> {
    ...
    let logs = log_dir();
    for name in ["spankd.err.log", "spankd.log"] {
...
mod tests {
    ...
    fn plist_is_well_formed_and_complete() {
        let text = plist(Path::new("/usr/local/bin/spankd"), Path::new("/tmp/logs"));

        assert!(text.starts_with("<?xml"));
        assert!(text.contains("<key>Label</key>"));
        assert!(text.contains("<string>com.chipcolate.spank</string>"));
        assert!(text.contains("<string>/usr/local/bin/spankd</string>"));
        assert!(text.contains("<string>--daemon</string>"));
        assert!(text.contains("/tmp/logs/spankd.log"));
    ...
    fn paths_with_xml_metacharacters_are_escaped() {
        let text = plist(
            Path::new("/Users/tom & jerry/bin/spankd"),
    ...
    fn replacing_into_a_missing_directory_creates_it() {
        ...
        replace_binary(&src, &dir.join("spankd")).unwrap();
        assert!(dir.join("spankd").exists());
