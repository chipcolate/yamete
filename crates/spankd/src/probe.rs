//! `spankd probe` — verify the sensor works on this machine and report stream health.
//!
...
use spank_sensor::{Access, Error, Imu, Sample, SensorKind, Stats};
...
pub fn run(secs: f64) -> Result<(), Error> {
    println!("Input Monitoring: {:?}", spank_sensor::check_access());
    if spank_sensor::check_access() == Access::Denied {
        println!("  (denied — requesting; approve the prompt and re-run)");
        spank_sensor::request_access();
    }
