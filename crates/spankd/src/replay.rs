//! `spankd replay` — run the detector over a recorded fixture.
//!
...
use spank_dsp::{Config, Detector, Fixture, Frame};
...
pub fn detect(fixture: &Fixture, cfg: Config) -> Vec<spank_dsp::Detection> {
...
pub fn decimate(fixture: &Fixture, n: usize) -> Fixture {
    ...
    let take_nth = |v: &Vec<spank_dsp::Frame>| v.iter().step_by(n).copied().collect();
    Fixture {
...
pub fn run(
    ...
    // and the pass/fail judgement lives in `spank-dsp`'s fixture tests against a measured
