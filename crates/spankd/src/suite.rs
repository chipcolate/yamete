//! `spankd record-suite` — walk through recording the whole fixture corpus.
//!
...
pub fn run(dir: &Path, only: Option<&str>) -> Result<(), Error> {
    ...
    println!("\nNow check them with:");
    println!("  cargo run -p spankd -- replay {}/*.fixture.gz -v", dir.display());
    println!("  cargo run -p spankd -- analyze {}/idle.fixture.gz", dir.display());
    Ok(())
}
