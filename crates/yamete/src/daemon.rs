...
pub fn run(foreground: bool, exit_with_parent: bool) -> Result<(), Error> {
    let config_path = yamete_proto::config_path();
...
fn detector_loop(
    ...
    let mut imu = Imu::open_with(yamete_sensor::DEFAULT_CAPACITY, config.report_interval_us)?;
    ...
    if foreground {
        println!(
            "yamete running — sensitivity {:.2}, {} action(s), gyro {}. Ctrl-C to stop.",
        ...
        if config_rx.has_changed().unwrap_or(false) {
            ...
            if config.report_interval_us != previous_interval {
                ...
                drop(std::mem::replace(
                    &mut imu,
                    Imu::open_with(yamete_sensor::DEFAULT_CAPACITY, config.report_interval_us)?,
                ));
                pump = Pump::new();
            }
...
mod tests {
    ...
    fn preview_slap_is_clamped_and_plausible() {
        ...
        assert_eq!(s.tier, yamete_dsp::Tier::Major);
    }
